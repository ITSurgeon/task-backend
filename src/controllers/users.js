    ...
    try {
      ...
      if (!userInviting.friends.includes(req.params.id)) {
        if (!userInviting.invitationsTo.includes(req.params.id)) {
          ...
          res.status(201).json(
            { message: `User ${userInvited.login} successfully invited` }
          );

        }
        return res.status(403).json(
          { message: `You have already invited user ${userInvited.login}` }
        );

      }
      return res.status(403).json(
        { message: `User ${userInvited.login} is already in your friends list` }
      );
...
const approveUser = async (req, res) => {
  try {
    ...
    if (!userApproving.friends.includes(userInviting.id)) {
      ...
      res.status(201).json(
        { message: `User ${userInviting.login} successfully added to friends` }
      );

    }
    return res.status(403).json(
      { message: `User ${userInviting.login} is allready in your friends list` }
    );
