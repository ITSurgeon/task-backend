...
const getOwnProfile = async function(id) {
  try {
    return await User.findById(id)
      .populate('friends')
      .populate('invitationsTo')
      .populate('invitationsFrom');
  } catch (e) {
    throw Error('Connection to DB failed');
  }
};
...
const inviteUser = async function(fromId, toId) {
  try {
    await User.findById(toId)
      .updateOne({ $push: { invitationsFrom: fromId } });
    await User.findById(fromId)
      .updateOne({ $push: { invitationsTo: toId } });
  } catch (e) {
    throw Error('Connection to DB failed');
  }
};
...
const approveUser = async function(fromId, toId) {
  try {
    await User.findById(fromId)
      .updateOne({ $push: { friends: toId } })
      .updateOne({ $pull: { invitationsFrom: toId } });
    await User.findById(toId)
      .updateOne({ $push: { friends: fromId } })
      .updateOne({ $pull: { invitationsTo: fromId } });
  } catch (e) {
    throw Error('Connection to DB failed');
  }
};
